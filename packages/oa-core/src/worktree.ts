...
export async function create(opts: CreateWorktreeOpts): Promise<WorktreeInfo> {
  ...
  // EEXIST collision check (Task 1.6 review carry-forward). We do NOT
  // auto-retry with a fresh id — the caller asked for THIS taskId and a
  // silent rename would obscure data-loss risk if the existing dir is alive.
  let exists = false;
  try {
